...
synchronized是一个重量级的锁，虽然JVM对它做了很多优化，volatile则是轻量级的synchronized。
...
假如有两个线程A、B都执行这个操作（i++），按照我们正常的逻辑思维主存中的 i 值应该=3，但事实是这样么？分析如下：
...
这个问题稍后回答，我们先看另一个原则happens-before。
...
![](media/14983989207383.jpg)

## 总结
volatile看起来简单，但是要想理解它还是比较难的，这里只是对其进行基本的了解。volatile相对于synchronized稍微轻量些，在某些场合它可以替代synchronized，但是又不能完全取代synchronized，只有在某些场合才能够使用volatile。使用它必须满足如下**两个条件**：
...
* 总结：[使用场合](http://blog.csdn.net/vking_wang/article/details/9982709)
